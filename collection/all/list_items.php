...
if (isset($_GET['danhMuc']) && empty($_GET['kichCo']) && empty($_GET['giaBan'])) {
    ...
    $kc = $_GET['kichCo'];
    $sql_sanpham = "SELECT * FROM sanpham LEFT JOIN chitietsanpham ON sanpham.idSanPham = chitietsanpham.idSanPham WHERE chitietsanpham.kichCo = '".$kc."'";
    $sql_trang = mysqli_query($conn, $sql_sanpham.=" AND sanpham.hienThi = 1");
    ...
    $sql_sanpham = "SELECT * FROM sanpham WHERE giaBan >= ".$gbd." AND giaBan <= ".$gkt."";
    $sql_trang = mysqli_query($conn, $sql_sanpham.=" AND sanpham.hienThi = 1");
    ...
    $gkt = 0;
    if ($gbd == '1'){$gbd = 0; $gkt = 99;} elseif ($gbd == '100'){ $gkt = 200;} elseif ($gbd == '200'){ $gkt = 300;}else{ $gkt = 10000;}
    $sql_sanpham = "SELECT * FROM sanpham LEFT JOIN chitietsanpham ON sanpham.idSanPham = chitietsanpham.idSanPham WHERE chitietsanpham.kichCo = '".$kc."' AND giaBan >= ".$gbd." AND giaBan <= ".$gkt."";
    $sql_trang = mysqli_query($conn, $sql_sanpham.=" AND sanpham.hienThi = 1");
    ...
    $sql_all = "SELECT * FROM sanpham WHERE hienThi = 1 GROUP BY tenSanPham ASC LIMIT $begin,12";
