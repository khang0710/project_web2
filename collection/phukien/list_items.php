...
if (isset($_GET['danhMuc']) && empty($_GET['kichCo']) && empty($_GET['giaBan'])) {
    ...
    WHERE danhmuc.tenDanhMuc LIKE '%Trang%' AND giaBan >= ".$gbd." AND giaBan <= ".$gkt." OR danhmuc.tenDanhMuc LIKE '%Mũ%' AND giaBan >= ".$gbd." AND giaBan <= ".$gkt."";
    $sql_trang = mysqli_query($conn, $sql_sanpham.=" AND sanpham.hienThi = 1");
    ...
    WHERE sanpham.hienThi = '1' AND  danhmuc.tenDanhMuc LIKE '%Trang%' OR danhmuc.tenDanhMuc LIKE '%Mũ%' LIMIT $begin,12";
    $sql_trang = mysqli_query($conn,"SELECT * FROM sanpham
    LEFT JOIN danhmuc ON sanpham.loaiSanPham = danhmuc.idDanhMuc
    WHERE sanpham.hienThi = '1' AND  danhmuc.tenDanhMuc LIKE '%Trang%' OR danhmuc.tenDanhMuc LIKE '%Mũ%'");
}
